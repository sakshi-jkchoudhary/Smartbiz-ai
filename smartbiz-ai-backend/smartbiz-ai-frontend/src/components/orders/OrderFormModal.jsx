...
export default function OrderFormModal({ isOpen, onClose, onSuccess }) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="New order" size="xl">
      {/* Container fully integrated with dynamic light/dark contrast layout utility sheets */}
      <form onSubmit={handleSubmit} className="space-y-5 text-slate-900 dark:text-slate-100 bg-white dark:bg-slate-900 transition-colors">
        
        {/* Customer & Payment Options Dropdown boxes */}
        <div className="grid grid-cols-2 gap-3">
          <Select label="Customer (optional)" value={customerId} onChange={(e) => setCustomerId(e.target.value)}>
            <option value="" className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">Walk-in customer</option>
            {customers.map((c) => (
          ...
          <Select label="Payment mode" value={paymentMode} onChange={(e) => setPaymentMode(e.target.value)}>
            ...
            <option value="pending" className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">Credit / PENDING</option>
          </Select>
        </div>

        {/* Product Browser Selection Area */}
        <div>
          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Add products</label>
          <div className="relative mb-2">
            ...
            <input
              ...
              className="input-field pl-9 w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl py-2 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto pr-1">
            {filteredProducts.map((p) => (
              <button
                ...
                className="flex items-center justify-between px-3 py-2 rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/20 hover:border-brand-300 hover:bg-brand-50/50 dark:hover:bg-slate-800 transition-colors text-left disabled:opacity-40 disabled:cursor-not-allowed"
              >
        ...
        {/* Selected Orders Cart Basket Component */}
        <div>
          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Order items</label>
          {cart.length === 0 ? (
            ...
            <div className="border border-slate-100 dark:border-slate-800 rounded-xl divide-y divide-slate-50 dark:divide-slate-800/60 max-h-40 overflow-y-auto bg-white dark:bg-slate-950/40">
              {cart.map((item) => (
        ...
        <div className="flex items-end gap-4">
          ...
          />
          <div className="flex-1 bg-slate-50 dark:bg-slate-950 rounded-xl px-4 py-3 flex items-center justify-between border border-slate-100 dark:border-slate-800/60">
